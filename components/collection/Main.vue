<template>
<CollectionList @openAddSidebar="openAddSidebar" @openEditSidebar="openEditSidebar" :projectData="projects" :key="render"/>
 <div v-if="addSidebar" :key="Addrender">
    <CollectionAdd @postData="postData"/>
  </div>
   <div v-if="editSidebar" :key="Editrender">
    <CollectionEdit :name="name" :listName="listName" :details="details" :specifications="specifications" :projectAge="projectAge" :logo="logo" :defaultImage="defaultImage"/>
  </div>
</template>
...
const openEditSidebar=(data:any)=>{
    ...
    listName.value=data[0].listing_type_name
    details.value=data[0].details
    specifications.value=data[0].specifications
    projectAge.value=data[0].age_of_the_project
    logo.value=data[0].logo_url
    defaultImage.value=data[0].default_image_url
    Editrender.value++
    editSidebar.value=true
}
async function postData(value:any) {
