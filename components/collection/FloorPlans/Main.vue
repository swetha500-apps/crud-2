<template>
<CollectionFloorPlansList @addfloor="addfloor" @editfloor="editfloor" @deletefloor="deletefloor" :floorPlans="floorPlans"/>
<CollectionFloorPlansAdd  v-if="isAdd" :key="addRender" @save="addCustomFloor"/>
<CollectionFloorPlansEdit v-if="isEdit" :key="editRender" :editPrefilledData="editPrefilledData" @saved="saved"/>
</template>
...
const props = defineProps({
  url: {
    type: String,
    default: "https://v1-orm-gharpe.mercury.infinity-api.net/api/floorplans/",   
  }
});
// Authorization token
const authHeader = {
...
//open edit sidebar
const editfloor = (editData:any) => {  
  isEdit.value = true;
  editPrefilledData.value=editData
     editRender.value++
}
// Get FloorPlans data using api call
const { data: floorPlans } = await useLazyFetch(
...
getData.value=floorPlans.value
// Add FloorPlans data
const addCustomFloor = async (postBody: any) => {
...
// Delete FloorPlans based on uid
const deletefloor = (uid: string,floorIndex:any) => {
  const { data: response } = useLazyFetch(`${props.url}${uid}`, {
    method: "DELETE",
    headers: authHeader,
  });
   getData.value.splice(floorIndex, 1);
};
</script>